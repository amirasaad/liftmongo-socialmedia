...
sealed trait UserSnippet extends SnippetHelper with Loggable {
  ...
  def title(xhtml: NodeSeq): NodeSeq = serve { user =>
    <lift:head>
      <title lift="Menu.title">{"Social Media: %*% - "+user.username.get}</title>
    </lift:head>
  }
}
        ...
        <ul class="nav navbar-nav navbar-right" id="user">
          <li class="dropdown" data-dropdown="dropdown">
            ...
            <ul class="dropdown-menu">
              <li><a href={Site.profileLoc.calcHref(user)}><i class="glyphicon glyphicon-user"></i> Profile</a></li>
              <li><lift:Menu.item name="Account" donthide="true" linktoself="true"><i class="glyphicon glyphicon-cog"></i> Settings</lift:Menu.item></li>
              <li class="divider"></li>
              <li><lift:Menu.item name="Logout" donthide="true"><i class="glyphicon glyphicon-off"></i> Log Out</lift:Menu.item></li>
            </ul>
          </li>
        </ul>
...
object AllUsers {
  ...
  def render  = {
   ".container *" #>  (users.map(u =>
     ".avatar *" #> (Gravatar.imgTag(u.email.get, 60))&
     ".username *" #> u.username.get &
     ".name *" #> u.name.get &
     ".bio *" #> u.bio.get &
      ".loc *" #> u.location.get &
     "* [href] " #> ("user/" + u.username.get)
     ))
  }
